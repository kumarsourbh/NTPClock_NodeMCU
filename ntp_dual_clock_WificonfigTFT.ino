...
// ESP based Device configuration for Wifi AP/SSID
...
const char   * t_zone = "Asia/Kolkata";     // See https://en.wikipedia.org/wiki/List_of_tz_database_time_zones
const char * ntp_pool = "in.pool.ntp.org";  // NTP Server pool address for India
const int  duty_cycle = 72;                 // TFT brightness using PWM duty cycle (0-255)
const int display_ori = 1;                  // TFT display orientation
String   new_hostname = "NTPSKUMAR";
...
void setup() {
  ...
  tft.setCursor(18, 10);                             // move cursor to position (18, 10) pixel
  tft.print("Please Connect to SSID!");
  tft.setCursor(18, 30);
  tft.print("Welcome Mr. SOURBH");
  ...
  // Connected!
  Serial.println("WiFi connected Amigos!");
